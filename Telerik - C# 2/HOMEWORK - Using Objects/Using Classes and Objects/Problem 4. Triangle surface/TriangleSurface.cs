...
namespace Problem_4.Triangle_surface
{
    ...
    {
        static decimal SidesAndAngle(decimal sideA, decimal sideB, decimal angleAB)
        {
            angleAB = angleAB * (decimal)Math.PI / 180.0m;
            return ((decimal)Math.Sin((double)angleAB) * sideA * sideB) / 2.0m;
        }

        static decimal HeightAndSide(decimal sideA, decimal heightToA)
        {
            return (sideA * heightToA) / 2.0m;
        }

        static decimal GivenSides(decimal sideA, decimal sideB, decimal sideC)
        {
